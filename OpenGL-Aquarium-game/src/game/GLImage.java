...
public class GLImage {
    ...
    // Used by app.drawImage() to hold image as a texture
    // The texture will be allocated only if needed (see app.drawImage()
    ...
    public GLImage(String imgName)
    {
        ...
        if (makeGLImage(img,true,false)) {
			app.msg("GLImage(String): loaded " + imgName + ", width=" + w + " height=" + h);
		}
    }
    ...
    public GLImage(String imgName, boolean flipYaxis, boolean convertPow2)
    {
        ...
        if (makeGLImage(img,flipYaxis,convertPow2)) {
			app.msg("GLImage(String,bool,bool): loaded " + imgName + ", width=" + w + " height=" + h);
		}
	}
    ...
    public GLImage(byte[] bytes) {
        ...
        if (makeGLImage(img,true,false)) {
			app.msg("GLImage(byte[]): loaded image from bytes[" + bytes.length + "]");
		}
		else {
			app.err("GLImage(byte[]): could not create Image from bytes[" + bytes.length + "]");
		}
    }
    ...
    public GLImage(byte[] bytes, boolean flipYaxis, boolean convertPow2) {
        BufferedImage img = makeBufferedImage(bytes);
        if (makeGLImage(img,flipYaxis,convertPow2)) {
			app.msg("GLImage(byte[],bool,bool): loaded image from bytes[" + bytes.length + "]");
		}
		else {
			app.err("GLImage(byte[],bool,bool): could not create Image from bytes[" + bytes.length + "]");
		}
    }
    ...
    public BufferedImage makeBufferedImage(byte[] imageFileContents) {
    	...
    	try {
    		...
    		app.err("GLImage.makeBufferedImage(): " + ioe);
    	}  
        return bi;
    }
    ...
    public boolean makeGLImage(BufferedImage tmpi, boolean flipYaxis, boolean convertToPow2) {
        if (tmpi != null) {
            ...
            textureW = app.getPowerOfTwoBiggerThan(w); // the texture size big enough to hold this image
            textureH = app.getPowerOfTwoBiggerThan(h); // the texture size big enough to hold this image
            //app.msg("GLImage: loaded " + imgName + ", width=" + w + " height=" + h);
            return true;
        }
        else {
            //app.err("GLImage: FAILED TO LOAD IMAGE " + imgName);
            pixels = null;
            pixelBuffer = null;
            h = w = 0;
            return false;
    ...
    public BufferedImage loadJavaImage(String imgName) {
    	...
    	try {
    		tmpi = ImageIO.read(app.getInputStream(imgName));
    	}
    	catch (Exception e) {
    		app.err("GLImage.loadJavaImage() exception: FAILED TO LOAD IMAGE " + e);
    	}
    	return tmpi;
	}
    ...
    public static int[] getImagePixels(Image image)
    {
        ...
        if (image != null) {
            ...
            try {
            	...
            	app.err("Pixel Grabbing interrupted!");
                return null;
            }
        }
        return pixelsARGB;
    ...
    public static ByteBuffer convertImagePixelsARGB(int[] jpixels, int imgw, int imgh, boolean flipVertically) {
    	...
    	ByteBuffer bb = app.allocBytes(jpixels.length * 4);  // 4 bytes per pixel
    ...
    /**
     * Same function as in app.java.  Allocates a ByteBuffer to hold the given
     * array of bytes.
     *
     * @param bytearray
	...
	public static void savePixelsToPNG(int[] pixels, int width, int height, String imageFilename, boolean flipY) {
		if (pixels != null && imageFilename != null) {
			...
			try {
				...
				app.err("GLImage.savePixelsToPNG(" +imageFilename+ "): exception " + e);
			}
		}
	}
    ...
    public static BufferedImage convertToPowerOf2(BufferedImage bsrc) {
        ...
        int newW = app.getPowerOfTwoBiggerThan(bsrc.getWidth());
        int newH = app.getPowerOfTwoBiggerThan(bsrc.getHeight());
