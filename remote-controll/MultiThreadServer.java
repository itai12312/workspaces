...
class ServerThread implements Runnable {
	...
	ClientTextArea sap;
	public ServerThread(Socket socket,ClientTextArea sta){
		this.socket=socket;
		this.sap=sta;
	}
		...
		try{
			...
			while (( line=in.readLine())!=null){
				if (!line.equals("Bye")){
					...
					RobotAccess.dealWith(line);
					if((int) (Math.random()*20)==15){
						sendImageToClient(RobotAccess.retbackground());
					}
				}
	...
	private sendImageToClient(BufferedImage retbackground) {
