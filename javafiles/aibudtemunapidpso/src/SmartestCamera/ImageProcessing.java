...
public class ImageProcessing {
	...
	public void evenmoretolerance(int[][]where,int[][]whattofind,ArrayList<Rect>potentislPoints,int tolerance){
		...
		for(int i=0;i<potentislPoints.size();i++){
			...
			else if(Math.abs(count(where,rect.getX(),rect.getY()+rect.getHeight()/2,rect.getLength()/2,rect.getHeight()/2)-target)>tolerance){
				potentislPoints.remove(i);
			}
			else if(Math.abs(count(where,rect.getX()+rect.getLength(),rect.getY(),rect.getLength()/2,rect.getHeight()/2)-target)>tolerance){
				potentislPoints.remove(i);
			}
			else if(Math.abs(count(where,rect.getX()+rect.getLength()/2,rect.getY()+rect.getHeight()/2,rect.getLength()/2,rect.getHeight()/2)-target)>tolerance){
				potentislPoints.remove(i);
			}
